...
class FoodServices {
  ...
  Future<ApiResult<PaginatedList<FoodVM>>> searchFood(
    ...
    IOClientWrapper ioClient = _httpClientFactory.createIOClientWrapper();
    String url =
        "$baseRoute/filter?pageNumber=${pagingRequest.pageNumber}&searchString=${pagingRequest.searchString}&sortOrder=${pagingRequest.sortOrder}";
    if (categoryIDs != null) {
  ...
  Future<ApiResult<PaginatedList<FoodVM>>> getBestSellingFoods() async {
    ...
    IOClientWrapper ioClient = _httpClientFactory.createIOClientWrapper();
    final String url =
        "$baseRoute/best_selling?pageNumber=${pagingRequest.pageNumber}&searchString=${pagingRequest.searchString}&sortOrder=${pagingRequest.sortOrder}";
    Response? response;
    try {
      log("GET: $url");
      response = await ioClient.get(
